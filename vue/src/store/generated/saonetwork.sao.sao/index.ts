...
import { Metadata } from "SaoNetwork-sao-client-ts/saonetwork.sao.sao/types"
...
import { QueryProposal } from "SaoNetwork-sao-client-ts/saonetwork.sao.sao/types"
import { RenewProposal } from "SaoNetwork-sao-client-ts/saonetwork.sao.sao/types"
import { ShardMeta } from "SaoNetwork-sao-client-ts/saonetwork.sao.sao/types"
import { TerminateProposal } from "SaoNetwork-sao-client-ts/saonetwork.sao.sao/types"


export { JwsSignature, Metadata, Params, PermissionProposal, Proposal, QueryProposal, RenewProposal, ShardMeta, TerminateProposal };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						...
						QueryProposal: getStructure(QueryProposal.fromPartial({})),
						RenewProposal: getStructure(RenewProposal.fromPartial({})),
						ShardMeta: getStructure(ShardMeta.fromPartial({})),
						TerminateProposal: getStructure(TerminateProposal.fromPartial({})),
...
export default {
	...
	getters: {
			...
			return state.Metadata[JSON.stringify(params)] ?? {}
				...
				getLatesthight: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.Latesthight[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryMetadata({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.SaonetworkSaoSao.query.queryMetadata(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.SaonetworkSaoSao.query.queryMetadata({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async QueryLatesthight({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.SaonetworkSaoSao.query.queryLatesthight()).data
				
					
				commit('QUERY', { query: 'Latesthight', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryLatesthight', payload: { options: { all }, params: {...key},query }})
				return getters['getLatesthight']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryLatesthight API Node Unavailable. Could not perform query: ' + e.message)
		...
		async sendMsgMigrate({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.SaonetworkSaoSao.tx.sendMsgMigrate({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgMigrate:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgMigrate:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgMigrate({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.SaonetworkSaoSao.tx.msgMigrate({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgMigrate:Init Could not initialize signing client. Wallet is required.')
				} else{
					throw new Error('TxClient:MsgMigrate:Create Could not create message: ' + e.message)
				}
			}
		},
