...
func networkWithShardObjects(t *testing.T, n int) (*network.Network, []types.Shard) {
	...
	return network.New(t, cfg), state.ShardList
}

func TestShowShard(t *testing.T) {
	net, objs := networkWithShardObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetShardResponse
...
func TestListShard(t *testing.T) {
	net, objs := networkWithShardObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListShard(), args)
			require.NoError(t, err)
			var resp types.QueryAllShardResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
